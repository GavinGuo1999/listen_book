...
- 新增 17 条结构化句子切分黄金用例，覆盖中文对白、连续标点、省略号、英文缩写、首字母、数字和中英混排。
- 切句器可完整保留 `？！` 等连续终止符，区分句内/句末省略号，并避免在内嵌英文引语中错误断句。
- 英文标题缩写、时间缩写、首字母组合、常见首字母缩略词和小数不再被错误拆分。
- TTS 根据句子中的中英文字符占比自动选择中文或英文 Edge Neural 音色。
- 英文 `whispered`、`shouted` 等对白提示词接入轻量语速和音高规则。
- 新增 11 条结构化 TTS 黄金用例，固定文本、音色、语速、rate 和 pitch 期望。
- 新增 `scripts/generate_tts_golden.py`，支持离线校验、按样例生成和强制重建。
- 生成器输出确定文件名、SHA-256 报告和移动端可用的本地试听索引；MP3 保留在忽略目录。
- 中英混排句会将引号内英文切换到英文音色，并为“那一行/第 N 行”等量词场景提供不改变显示文本的读音别名。
- Edge TTS 模型规则版本升级到 `15`，避免继续复用旧规则生成的缓存。

## v0.5.0 - 2026-07-22

- 管理后台重构为系统状态、书籍审核、用户管理、任务队列和操作审计五个标签页。
- 新增数据库连接、Alembic 版本、存储写权限、会话密钥、管理员配置和 Worker 在线诊断。
- Worker 定期写入持久化心跳，管理页面每 15 秒更新在线状态。
- 新增命令行诊断脚本 `scripts/check_system.py`，输出不包含密钥、密码和本地路径。
- 用户管理支持搜索、状态/角色筛选、分页、启停账号、授予/撤销管理员和上传记录查看。
- 禁止修改当前登录账号，保护 bootstrap 管理员，并确保至少保留一个启用中的管理员。
- 用户启停和管理员权限变更写入独立管理员操作审计表。
- 书籍审核支持按书名/上传者搜索、选择本页和批量批准/拒绝。
- 批量审核保持逐书 `book_review_events` 审计记录，并在缺失书籍时整体拒绝操作。
- 新增管理员 API、服务层规则、迁移、后端测试和浏览器 E2E 覆盖。
...
- 阅读体验：用户级音色/语速设置、连续播放下一章和睡眠定时器。
- 运维增强：数据库与书籍文件备份、缓存容量统计和 GitHub Actions。
