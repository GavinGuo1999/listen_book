...
## 最近交接记录：2026-07-16（v0.5.0 验收版）

本轮完成：

- 新增 Alembic 迁移 `20260715_0005`，创建 `worker_heartbeats` 和 `admin_audit_events`。
- Worker 每 5 秒写入心跳；系统状态按默认 30 秒阈值判断在线/离线。
- 新增系统诊断：数据库连接、Alembic 版本、存储写权限、会话密钥、管理员配置和 Worker 心跳。
- 新增 `scripts/check_system.py`，诊断输出不包含密钥、密码、数据库连接串或存储绝对路径。
- 管理后台重构为系统状态、书籍审核、用户管理、任务队列和操作审计五个标签页。
- 用户管理支持搜索、状态/角色筛选、分页、上传记录、启停账号和管理员授权。
- 禁止修改当前登录账号，保护 bootstrap 管理员，并保留最后一个启用中的管理员。
- 用户启停和管理员权限变更写入独立审计事件。
- 书籍审核增加书名/上传者搜索、选择本页和批量批准/拒绝；每本书继续写独立审批事件。
- 新增 `docs/v0.5.0-acceptance.md` 人工验收清单。
- 前后端及 FastAPI 版本统一升级为 `0.5.0`。
- 本机开发 PostgreSQL 已迁移到 `20260715_0005 (head)`。

本轮验证：

- `.venv\Scripts\python.exe -m pytest backend\tests -q`：`41 passed`。
- 全量 Ruff：通过。
- `cd frontend && npm run build`：通过，版本 `0.5.0`。
- `cd frontend && npm run test:e2e:sqlite`：`6 passed`。
- 隔离浏览器检查：桌面和约 390px 窄屏无页面级横向溢出，控制台无错误。

人工验收入口：

1. 运行 `scripts\stop-dev.bat` 和 `scripts\start-dev.bat`。
2. 打开 `http://127.0.0.1:5173/`，管理员登录后进入 `/admin`。
3. 按 `docs/v0.5.0-acceptance.md` 检查诊断、批量审批、用户管理和审计。
