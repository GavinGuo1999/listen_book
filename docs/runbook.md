...
后端 pytest 额外覆盖 TXT、GB18030 TXT、EPUB 解析、注册/登录、用户进度隔离、公共书库审批可见性、阅读进度、删除清理、音频接口、TTS 语气规则和失败路径。

## E2E 浏览器测试

Playwright E2E 测试覆盖注册/登录/退出、上传、解析等待、阅读进度恢复、删除、普通用户上传待审、管理员待审批列表批准和批准后公共可见等关键路径。当前脚本刻意不触发真实 TTS，避免 E2E 依赖外部网络。
...
- 管理员可在前端独立“待审批书籍”列表里集中处理待审上传，也可在书库行点击“批准/拒绝”。
- 拒绝时可填写审批备注；备注会保存到书籍的 `review_note`。
- 也可直接调用：
